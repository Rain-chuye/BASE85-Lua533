...
static int convert(lua_State *L, unsigned (*conv)(unsigned)) {
  ...
  if (t == LUA_TNUMBER)
    ...
    return luaL_error(L, "期望为 number/string，得到的是 %s", luaL_typename(L, 1));
  else {
    luaL_Buffer b;
...
static int push_offset(lua_State *L, const char *s, const char *e,
  ...
  const char *next = utf8_next(cur, e);
...
static const char *classend (MatchState *ms, const char *p) {
  ...
  switch (ch) {
    case L_ESC: {
      if (p == ms->p_end)
        luaL_error(ms->L, "模式格式错误 (以 " LUA_QL("%%") ")");
    ...
    case '[': {
      ...
      do {  /* look for a `]' */
        if (p == ms->p_end)
          luaL_error(ms->L, "模式格式错误 (缺少 " LUA_QL("]") ")");
...
static int push_captures (MatchState *ms, const char *s, const char *e) {
  ...
  luaL_checkstack(ms->L, nlevels, "捕获太多");
  for (i = 0; i < nlevels; i++)
