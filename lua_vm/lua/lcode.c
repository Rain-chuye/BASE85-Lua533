...
static int addk (FuncState *fs, TValue *key, TValue *v) {
  ...
  if (ttisnil(idx)) {
    ...
    idx = luaH_get(fs->ls->h, key);
    luaM_growvector(L, f->k, fs->nk, f->sizek, TValue, MAXARG_Ax, "constants");
    while (fs->nk < f->sizek) setnilvalue(&f->k[fs->nk++]);
    fs->nk--;
    setobj2s(L, &f->k[fs->nk], v);
    fs->nk++;
    luaC_barrier(L, f, v);
  }
