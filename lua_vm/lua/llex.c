...
static const char *const luaX_tokens [] = {
    ...
    "in", "lambda", "local", "global", "nil", "not", "or", "repeat",
    "global",
...
TString *luaX_newstring (LexState *ls, const char *str, size_t l) {
  ...
  o = cast(TValue *, luaH_get(ls->h, L->top - 1));
  if (o == luaO_nilobject) {
    o = luaH_newkey(L, ls->h, L->top - 1);
    setbvalue(o, 1);
    luaC_checkGC(L);
  }
  else {
...
static int gethexa (LexState *ls) {
  save_and_next(ls);
  esccheck (ls, lisxdigit(ls->current), "期望十六进制数字");
...
//---
  while ((save_and_next(ls), lisxdigit(ls->current))) {
    i++;
    esccheck(ls, r <= (0x7FFFFFFFu >> 4), "UTF-8 值太大");
...
static int readdecesc (LexState *ls) {
  ...
  esccheck(ls, r <= UCHAR_MAX, "十进制转义太大");
...
//mod by nirenr
      ...
      case '&': {
         ...
         if (check_next1(ls, '=')) return TK_BITANDEQ;
         return '&';
      }
      case '|': {
         next(ls);
         if (check_next1(ls, '|')) return TK_OR;
         if (check_next1(ls, '=')) return TK_BITOREQ;
         return '|';
      }
//---
      case '~': {
...
void luaX_next (LexState *ls) {
  ...
  if (ls->lookahead.token != TK_EOS) {  /* is there a look-ahead token? */
    ...
    ls->lookahead = ls->lookahead2;  /* shift next lookahead */
    ls->lookahead2.token = TK_EOS;
  }
  else
