...
static l_noret error_expected (LexState *ls, int token) {
  luaX_syntaxerror(ls,
      luaO_pushfstring(ls->L, "期望出现 %s", luaX_token2str(ls, token)));
}
      ...
      switch (ls->t.token) {
        case TK_NAME: {  /* param -> NAME */
          ...
          if (ls->t.token == TK_NAME) {
            new_localvar(ls, str_checkname(ls));
            ls->dyd->actvar.arr[ls->dyd->actvar.n - 1].kind = RDKVAVAR;
            f->is_vararg = 2; // Named vararg table
            nparams++;
          } else {
            f->is_vararg = 1;
          }
...
static void check_readonly (LexState *ls, expdesc *e) {
  if (e->k == VLOCAL) {
    Vardesc *vardesc = &ls->dyd->actvar.arr[e->u.info];
    if (vardesc->kind == VDKFOR)
      luaX_syntaxerror(ls, "attempt to assign to read-only for-loop variable");
  }
}
...
static void fornum (LexState *ls, TString *varname, int line) {
  ...
  new_localvar(ls, varname); ls->dyd->actvar.arr[ls->dyd->actvar.n - 1].kind = VDKFOR;
  checknext(ls, '=');
...
static void forlist (LexState *ls, TString *indexname) {
  ...
  new_localvar(ls, indexname); ls->dyd->actvar.arr[ls->dyd->actvar.n - 1].kind = VDKFOR;
  while (testnext(ls, ',')) {
    new_localvar(ls, str_checkname(ls)); ls->dyd->actvar.arr[ls->dyd->actvar.n - 1].kind = VDKFOR;
    nvars++;
  }
...
static void globalstat (LexState *ls) {
  ...
  do {
    ...
    /* In a real 5.5, this would mark the name as global */
    (void)name;
  } while (testnext(ls, ','));
}
...
static void statement (LexState *ls) {
  ...
  switch (ls->t.token) {
    ...
    case TK_GLOBAL: globalstat(ls); break; case TK_LOCAL: {  /* stat -> localstat */
